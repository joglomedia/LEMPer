...
function init_certbotle_removal() {
    ...
    while [[ "${REMOVE_CERTBOTCONF}" != "y" && "${REMOVE_CERTBOTCONF}" != "n" && "${AUTO_REMOVE}" != true ]]; do
        read -rp "Remove Certbot config and LE certificate files? [y/n]: " -e REMOVE_CERTBOTCONF
    done
    if [[ "${REMOVE_CERTBOTCONF}" == Y* || "${REMOVE_CERTBOTCONF}" == y* || "${FORCE_REMOVE}" == true ]]; then
        if [ -d /etc/letsencrypt ]; then
            run rm -fr /etc/letsencrypt
        fi
        echo "All your Redis Certbot config and LE certificate files deleted permanently."
    fi

    # Final test.
    if "${DRYRUN}"; then
        warning "Certbot Let's Encrypt removed in dryrun mode."
    else
        if [[ -z $(command -v certbot) ]]; then
            status "Certbot Let's Encrypt removed succesfully."
        else
            warning "Unable to remove Certbot Let's Encrypt."
        fi
    fi
}
...
if [[ -n $(command -v certbot) ]]; then
    while [[ "${REMOVE_CERTBOT}" != "y" && "${REMOVE_CERTBOT}" != "n" && "${AUTO_REMOVE}" != true ]]; do
        read -rp "Are you sure to remove Redis server? [y/n]: " -e REMOVE_CERTBOT
    done
